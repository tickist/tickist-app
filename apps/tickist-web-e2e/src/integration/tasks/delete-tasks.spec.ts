...
describe("Delete task", () => {
    ...
    beforeEach(() => {
        ...
        cy.get("@database").then((database: any) => {
            ...
            cy.callFirestore(
                ...
                JSON.parse(JSON.stringify(deletedTask))
            );
            ...
            cy.callFirestore(
                "set",
                `tasks/${nonDeletedTask.id}`,
                JSON.parse(JSON.stringify(nonDeletedTask))
            );
    ...
    it('should delete task after click on button "delete task" and "Yes"', () => {
        cy.get(`tickist-single-task:contains("${deletedTaskName}")`, {
            ...
            cy.wrap($task.find("#first-row"))
                .trigger("mouseenter")
                .wrap($task.find('[data-cy="task-short-menu"]'))
                .click();
        ...
        cy.get("simple-snack-bar")
            .contains("Task has been deleted successfully")
            .should("exist");
        cy.get(`tickist-single-task:contains("${deletedTaskName}")`).should(
            "not.exist"
        );
    });
    ...
    it('should not delete task after click on button "delete task" and "No"', () => {
        ...
        cy.get(`tickist-single-task:contains("${nonDeletedTaskName}")`).should(
            "exist"
        );
    });
});
