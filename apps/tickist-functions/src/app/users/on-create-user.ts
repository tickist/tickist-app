...
import {db} from "../init";
import {Project, ProjectType, ShareWithUser, Tag, Task, TaskType} from "@data";

export const onCreateUser = functions.firestore.document("users/{userId}").onCreate(async (snap, context) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        transaction.set(
            ...
            JSON.parse(
                JSON.stringify(
                    createTask("Find out more about Tickist", snap.data(), inbox, [tags["quick tasks"], tags["getting to know Tickist"]])
                )
            )
        );
        ...
        transaction.set(
            ...
            JSON.parse(
                JSON.stringify(
                    createTask("Find out more about editing tasks", snap.data(), inbox, [
                        tags["work"],
                        tags["home"],
                        tags["need focus"],
                        tags["someday/maybe"],
...
function defaultTagsName() {
    return ["work", "home", "need focus", "someday/maybe", "tasks for later", "quick tasks", "getting to know Tickist"];
}
