...
export const onCreateUser = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        return db.runTransaction(async (transaction) => {
            ...
            const inbox = createInboxProject(
                snap.data(),
                userId,
                projectRef.id
            );
            transaction.set(
                projectRef,
                JSON.parse(JSON.stringify({ ...inbox }))
            );
            defaultTagsName().forEach((tagName) => {
                ...
                transaction.set(
                    tagRef,
                    JSON.parse(JSON.stringify(createTag(tagName, userId)))
                );
            ...
            transaction.set(
                ...
                JSON.parse(
                    JSON.stringify(
                        createTask(
                            "Find out more about editing tasks",
                            snap.data(),
                            inbox,
                            [
                                tags["work"],
