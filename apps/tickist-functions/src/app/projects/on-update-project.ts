...
export const onUpdateProject = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        const projectHistoryRef = change.after.ref
            .collection("history")
            .doc(timeStamp);
            ...
            for (const task of query) {
                if (task.data().owner.id === beforeData.owner) {
                    ...
                    const owner = await db
                        .collection("users")
                        .doc(task.data().owner.id)
                        .get();
                    const inbox = await db
                        .collection("projects")
                        .doc(owner.data().inboxPk)
                        .get();
        ...
        const beforeDataInviteUserByEmail = beforeData.inviteUserByEmail
            ? beforeData.inviteUserByEmail.length
            : 0;
        if (beforeDataInviteUserByEmail < afterData.inviteUserByEmail.length) {
            ...
            for (const entry of inviteUserByEmail) {
                ...
                if (user && !beforeData.shareWithIds.includes(user.id)) {
                    ...
                    const index = newInviteUserByEmail.findIndex(
                        (invitedUser) => invitedUser.email === email
                    );
                    if (index === -1) {
                        ...
                        newInviteUserByEmail = newInviteUserByEmail.map(
                            (invitedUser) => {
...
export const createUpdateProjectNotifications = functions.firestore
    .document("projects/{projectId}")
    .onUpdate(async (change, context) => {
        const before = change.before;
        const after = change.after;
        ...
        if (!equals(beforeData.shareWithIds, afterData.shareWithIds)) {
            for (const userId of beforeData.shareWithIds) {
                if (!afterData.shareWithIds.includes(userId)) {
                    if (userId === editor.id) {
                        ...
                        const recipients = afterData.shareWithIds.filter(
            ...
            for (const userId of afterData.shareWithIds) {
                if (
                    !beforeData.shareWithIds.includes(userId) &&
                    userId !== editor.id
                ) {
