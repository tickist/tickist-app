...
export const selectActiveDateState =
    createFeatureSelector<ActiveDateState>("activeDate");
...
export const selectActiveDate = createSelector(
    ...
    (activeDateState) => {
        ...
        if (activeDateState.active.state === StateActiveDateElement.future) {
            date = parse(
                `01-${activeDateState.active.date}`,
                "dd-LLLL-uuuu",
                new Date()
            );
        } else if (
            activeDateState.active.state === StateActiveDateElement.weekdays
        ) {
            const splittedDate: string[] =
                activeDateState.active.date.split("-");
            date = new Date(
