...
describe("Task selectors", () => {
    ...
    describe("#projectWithoutNextActionTasks", () => {
        ...
        it("should return projects without next action task type", () => {
            expect(
                projectWithoutNextActionTasks.projector(tasks, projects)
            ).toStrictEqual([projects[1]]);
        });
    });
    ...
    describe("#nextActionTasks", () => {
        ...
        beforeEach(() => {
            tasks = [
                {
                    id: "1",
                    taskProject: { id: "1" } as TaskProject,
                    taskType: TaskType.normal,
                },
                {
                    id: "2",
                ...
                {
                    id: "3",
                    taskProject: { id: "2" } as TaskProject,
                    taskType: TaskType.needInfo,
                },
            ];
        });
    ...
    describe("#nextActionTasksLength", () => {
        ...
        it("should return how many tasks (type nextAction) do we have", () => {
            expect(nextActionTasksLength.projector(tasks)).toStrictEqual(
                tasks.length
            );
        });
    });
    ...
    describe("#needInfoTasksLength", () => {
        ...
        it("should return how many tasks (type needInfo) do we have", () => {
            expect(needInfoTasksLength.projector(tasks)).toStrictEqual(
                tasks.length
            );
        });
    });
    ...
    describe("#projectWithoutNextActionTasksLength", () => {
        ...
        it("should return how many projects without next action type tasks do we have", () => {
            expect(
                projectWithoutNextActionTasksLength.projector(projects)
            ).toStrictEqual(projects.length);
        });
    });
});
