...
export class TasksFiltersEffects {
    changeFiltersOnRouterChange$ = createEffect(() =>
        this.actions$.pipe(
            ...
            filter((action) => action.payload.event.url.indexOf("tasks") > 0 || action.payload.event.url.indexOf("tags") > 0),
            ...
            concatMap(([action]) => {
                ...
                actions.push(
                    ...
                    setCurrentAssignedToFilter({
                        currentFilter: TasksFiltersService.getAssignedToAllFilter(),
                    }),
