...
OAuth 2.1 authorization-code access uses PKCE through Supabase Auth. The Worker publishes RFC 9728 protected-resource metadata and authorization-server metadata, accepts tokens only when issuer, audience, expiry, subject, live user, `tickist_mcp`, and the signed `tickist_mcp_scopes` claim checks pass, and forwards the user's Bearer token to Supabase PostgREST. The browser consent route is `/auth/oauth/consent`; connected grants are listed and revoked at `/app/settings/connected-apps`. Supabase dynamic client registration remains enabled for client compatibility.
...
The production workflow configures and verifies the OAuth server, dynamic registration, consent path, and custom access-token hook after migrations. It temporarily deploys the personal-token compatibility Edge Function, then deploys the `tickist-app` and `tickist-mcp` Cloudflare Workers separately. Each run dynamically registers a public smoke client, completes authorization code with S256 PKCE and explicit consent through a dedicated test user, validates the fresh OAuth JWT with modern and legacy requests, optionally checks a scoped personal token when `MCP_PERSONAL_SMOKE_TOKEN` is configured, and revokes the smoke grant. Adding that secret enables the compatibility check automatically. Production runs on `master` pushes or manual workflow dispatch; a `develop` push alone does not deploy either Worker.
