...
Edge Functions handle reminders, shared-project updates, invitations, notification digests, outbox enqueueing, email delivery, routines, and MCP integration. The MCP function is a dual-era Streamable HTTP server: stateless protocol `2026-07-28` is the primary contract and initialization-based `2025-06-18` remains available for compatibility. Sensitive functions use `INTERNAL_FUNCTION_SECRET`, a validated user JWT, or a hashed personal API token as appropriate. MCP API-token scopes are enforced per tool, project references are checked before service-role writes, and database diagnostics are not returned to clients. AWS SES credentials remain in Edge Function secrets.

## Cloudflare deployment

The Worker serves built SPA assets, runtime configuration, and the dedicated `/mcp` endpoint. The MCP proxy validates browser origins and preserves authorization, protocol-version, method, name, and MCP parameter headers before forwarding requests to Supabase. The production pipeline deploys the Supabase MCP function before the Cloudflare proxy and smoke-tests the public `/mcp` route after both deployments. SPA fallback is enabled for application routes. For the public blog indexes (`/en/blog` and `/pl/blog`), it injects locale-specific title, description, canonical, robots, Open Graph, Twitter-card, and JSON-LD metadata into the initial HTML response. The Worker must not turn private API or workspace paths into indexable public content.
