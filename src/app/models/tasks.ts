...
export class Task extends Api {
  ...
  constructor(task) {
    ...
    this.finishTime = this.setFinishTime(task.finish_time);
  ...
  setFinishTime(finishTime: string) {
    if (finishTime) {
      const date = new Date();
      date.setHours(parseInt(finishTime.split(':')[0], 10), parseInt(finishTime.split(':')[1], 10),
        parseInt(finishTime.split(':')[2], 10));
      return date;
    } else {
      return null;
    }
