...
export class EstimateTimeDialogComponent {
    ...
    changeEstimateTime() {
        if (this.estimateTime__ltValues.length > 0 && this.estimateTime__gtValues.length > 0) {
            this.estimateTime__ltValue = this.estimateTime__ltValues.filter(
                elem => elem.label === 'estimateTime__lt' && elem.id === this.rangeValues[0]
            )[0];
            this.estimateTime__ltId = this.estimateTime__ltValue['id'];

            this.estimateTime__gtValue = this.estimateTime__gtValues.filter(
                elem => elem.label === 'estimateTime__gt' && elem.id === this.rangeValues[1]
            )[0];
            this.estimateTime__gtId = this.estimateTime__gtValue['id'];
