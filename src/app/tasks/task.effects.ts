...
import {allTasksLoaded, selectAllTasks, selectTaskById} from './task.selectors';
...
export class TaskEffects {
    ...
    updateTask$ = this.actions$
        .pipe(
            ...
            mergeMap((action) => this.store.select(selectTaskById(<number> action.payload.task.id))),
            mergeMap((task: Task, index: number) => this.tasksService.updateTask(task)),
