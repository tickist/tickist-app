...
export class TaskService {
  ...
  loadTasksFilters(user: User) {
    const filters = [
      ...
      {'id': 3, 'label': 'filter', 'name': 'w/o estimated time', 'value': task => task.estimateTime !== undefined},
      ...
      },
      {
        ...
        'value': task => true,
        'name': 'all',
        'avatar': '/assets/defaul_avatar.png',
        fixed: true
      },
  ...
  postponeToToday() {
    this.http.post<Task[]>(`${environment['apiUrl']}/tasks/move_tasks_for_today/`, {}).
